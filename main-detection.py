...
model_path = 'C:/v12/runs/detect/train5/weights/best.pt'  # Atualize com o caminho do modelo de detecção
...
# Função para detectar objetos e retornar a classe dominante
def detect_image(image_path, model):
    results = model.predict(image_path, save=False, save_txt=False)
    if results[0].boxes:
        # Obtém o ID da classe com maior confiança
        class_ids = results[0].boxes.cls.tolist()  # IDs das classes detectadas
        confidences = results[0].boxes.conf.tolist()  # Confianças correspondentes
        top_idx = confidences.index(max(confidences))  # Índice do bounding box com maior confiança
    ...
    if uploaded_files:
        ...
        for idx, uploaded_file in enumerate(uploaded_files):
            with columns[idx % 5]:
                ...
                class_id, confidence = detect_image(image_path, model)

                # Exibir o resultado
                if class_id is not None:
                    st.image(image_path, caption=f"Classe: {class_names[class_id]} ({confidence:.2%})", width=200)
                else:
                    st.image(image_path, caption="Nenhum objeto detectado", width=200)
